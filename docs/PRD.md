...
代理只能由用户明确启用。V0.2 不在打开新终端时自动启用代理。
...
V0.2 不收集遥测数据，不上传配置，不记录用户执行的目标命令。
...
- V0.2 不提供交互式配置向导；提供非交互式 `prox config path/show/init/validate` 命令。
...
V0.2 不通过大量专用环境变量覆盖配置，避免再次引入隐式状态。
...
V0.2 不主动设置大写 `HTTP_PROXY`，也不主动删除用户已有的 `HTTP_PROXY`。
...
- V0.2 中，执行 `prox off` 仍恢复启用前快照，并明确提示检测到变量漂移。
...
- V0.2 的人类可读文本不作为稳定机器接口；脚本应依据退出状态。
...
即使 V0.2 不支持代理认证，也不得在错误输出中完整泄露密码。
...
- Linux amd64/arm64 的 `prox` 可执行文件；
- tar.gz、deb 和 rpm 软件包；
- SHA-256 校验和；
- Bash 集成方式；
- Bash completion；
- 默认配置说明；
